        ...
        {franchises.map(fr => (
          <div key={fr.id} style={{ background:brand.card, border:'1px solid '+brand.border, borderLeft:'4px solid '+statusColor(fr.status), borderRadius:12, padding:16 }}>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8 }}>
              <div><div style={{ fontWeight:700, color:brand.heading }}>{fr.name.replace('TheValueStore — ','')}</div><div style={{ fontSize:11, color:brand.dim }}>{fr.owner} · {fr.city} · Since {fr.since}</div></div>
