...
export const defaultKynetraTemplates = [
  ...
  { id: 't13', name: 'Budget PC', module: 'pre_sales', triggerKeywords: ['budget', 'cheap', 'under 50k', '50k', 'affordable'], replyText: "Opening Build PC. Set your budget with the slider and use presets like ‘Max Value’ or ‘Gaming 1080p’.", action: 'open_buildpc', actionPayload: { tab: 'buildpc', preset: 'budget', presetId: 'BG005' }, suggestedReplies: ['Open Build PC', 'Show menu'], active: true, order: 13 },
  ...
  { id: 't40', name: 'Quiet PC', module: 'build_pc', triggerKeywords: ['quiet', 'silent', 'noise'], replyText: "In Build PC use the ‘Silent / Quiet’ preset and we’ll suggest low-noise parts.", action: 'open_buildpc', actionPayload: { tab: 'buildpc', preset: 'silent', presetId: 'BG009' }, suggestedReplies: ['Budget build', 'Compatibility'], active: true, order: 40 },
  { id: 't41', name: 'Streaming PC', module: 'pre_sales', triggerKeywords: ['streaming', 'stream', 'content creation'], replyText: "Opening Build PC. Use the ‘Streaming + Gaming’ preset for a balanced streaming build.", action: 'open_buildpc', actionPayload: { tab: 'buildpc', preset: 'streaming', presetId: 'BG003' }, suggestedReplies: ['Gaming PC', 'Budget build'], active: true, order: 41 },
  { id: 't42', name: 'Workstation', module: 'pre_sales', triggerKeywords: ['workstation', 'work', 'professional', 'creator'], replyText: "Opening Build PC. Use the ‘Workstation / Creator’ preset for professional builds.", action: 'open_buildpc', actionPayload: { tab: 'buildpc', preset: 'workstation', presetId: 'BG008' }, suggestedReplies: ['Build PC', 'Show menu'], active: true, order: 42 },
  { id: 't43', name: 'SFF / small', module: 'build_pc', triggerKeywords: ['small', 'sff', 'mini', 'compact'], replyText: "Opening Build PC. Use the ‘Small Form Factor’ preset for compact builds.", action: 'open_buildpc', actionPayload: { tab: 'buildpc', preset: 'sff', presetId: 'BG004' }, suggestedReplies: ['Compatibility', 'Budget build'], active: true, order: 43 },
...
export function resolveAction(template) {
  ...
  if (a === 'navigate' && p.tab) return { type: 'navigate', tab: p.tab, category: p.category, preset: p.preset, presetId: p.presetId };
  if (a === 'open_buildpc') return { type: 'navigate', tab: 'buildpc', preset: p.preset, presetId: p.presetId };
